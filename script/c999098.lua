...
function s.costfilter(c,opmonsters)
	...
	local atk=math.max(0,c:GetAttack())
	local def=math.max(0,c:GetDefense())
	local higher=math.max(atk,def)
	for oc in aux.Next(opmonsters) do
		local oatk=oc:GetAttack()
		if oc:IsFaceup() and higher>oatk then
			return true
		end
	end
	return false
