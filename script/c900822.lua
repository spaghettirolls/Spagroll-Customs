...
function s.sfilter(c,tp)
	return c:IsMonster() and c:IsSetCard(0x3D4) and c:IsAbleToGrave()
end
...
function s.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsPlayerCanSpecialSummonMonster(tp,id,0,0x21,1500,1500,4,RACE_FIEND,ATTRIBUTE_DARK) end
...
function s.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	Duel.BreakEffect()
local g=Duel.GetMatchingGroup(s.sfilter,tp,LOCATION_DECK+LOCATION_EXTRA,0,nil)
