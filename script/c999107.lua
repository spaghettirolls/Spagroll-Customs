...
function s.initial_effect(c)
    ...
    e2:SetTarget(aux.TargetBoolFunction(Card.IsSetCard,SET_NECROID))
	e2:SetValue(function(_,_,r) return (r&REASON_BATTLE==REASON_BATTLE) and 1 or 0 end)
	c:RegisterEffect(e2)
end
...
function s.spconfilter(c)
	return c:IsPreviousLocation(LOCATION_MZONE) or (not c:IsPreviousLocation(LOCATION_ONFIELD) and c:IsMonster()) and c:IsSetCard(0x238C)
end
